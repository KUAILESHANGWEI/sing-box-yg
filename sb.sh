...
if [ -z "$menu" ] || [ "$menu" = "1" ] ; then
	sbcore=$(curl -Ls https://api.github.com/repos/KUAILESHANGWEI/sing-box-yg/releases | grep -oP '"tag_name":\s*"v\K[0-9.]+' | head -n 1)
else
sbcore='1.10.7'
fi
sbname="sing-box-$sbcore-linux-$cpu"
curl -L -o /etc/s-box/sing-box.tar.gz  -# --retry 2 https://github.com/KUAILESHANGWEI/sing-box-yg/releases/download/v$sbcore/$sbname.tar.gz
...
bash <(curl -Ls https://raw.githubusercontent.com/KUAILESHANGWEI/sing-box-yg/main/vendor/acme.sh)
if [[ ! -f /root/ygkkkca/cert.crt && ! -f /root/ygkkkca/private.key && ! -s /root/ygkkkca/cert.crt && ! -s /root/ygkkkca/private.key ]]; then
        ...
        "rule_set": [
            {
                ...
                "type": "remote",
                "format": "binary",
                "url": "https://github.com/KUAILESHANGWEI/sing-box-yg/releases/download/meta-rules-dat-srs/geosite-geolocation-cn.srs",
            ...
            },
            {
                ...
                "type": "remote",
                "format": "binary",
                "url": "https://github.com/KUAILESHANGWEI/sing-box-yg/releases/download/meta-rules-dat-srs/geoip-cn.srs",
                "download_detour": "direct"
            }
        ],
        "final": "proxy",
...
curl -L -o /etc/s-box/cloudflared -# --retry 2 https://github.com/KUAILESHANGWEI/sing-box-yg/releases/download/cloudflared/cloudflared-linux-$cpu
...
wget -q -O /root/geoip.db https://github.com/KUAILESHANGWEI/sing-box-yg/releases/download/meta-rules-dat/geoip.db
wget -q -O /root/geosite.db https://github.com/KUAILESHANGWEI/sing-box-yg/releases/download/meta-rules-dat/geosite.db
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
curl -sL https://raw.githubusercontent.com/KUAILESHANGWEI/sing-box-yg/main/version | awk -F "更新内容" '{print $1}' | head -n 1 > /etc/s-box/v
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
lnsb(){
rm -rf /usr/bin/sb
curl -L -o /usr/bin/sb -# --retry 2 --insecure https://raw.githubusercontent.com/KUAILESHANGWEI/sing-box-yg/main/sb.sh
chmod +x /usr/bin/sb
}
...
lapre(){
json=$(curl -Ls --max-time 5 https://api.github.com/repos/KUAILESHANGWEI/sing-box-yg/releases)
latcore=$(echo "$json"|grep -oP '"tag_name":\s*"v\K[0-9.]+'|head -n1)
precore=$(echo "$json"|grep -oP '"tag_name":\s*"v\K[0-9.]+-[^"]+'|head -n1)
...
readp "请选择【0-3】：" menu
if [ "$menu" = "1" ]; then
upcore=$(curl -Ls https://api.github.com/repos/KUAILESHANGWEI/sing-box-yg/releases | grep -oP '"tag_name":\s*"v\K[0-9.]+' | head -n 1)
elif [ "$menu" = "2" ]; then
upcore=$(curl -Ls https://api.github.com/repos/KUAILESHANGWEI/sing-box-yg/releases | grep -oP '"tag_name":\s*"v\K[0-9.]+-[^"]+' | head -n 1)
elif [ "$menu" = "3" ]; then
echo
red "注意: 版本号在 https://github.com/KUAILESHANGWEI/sing-box-yg/releases 可查，且有Downloads字样 (必须1.10系或者1.30系以上版本)"
...
curl -L -o /etc/s-box/sing-box.tar.gz  -# --retry 2 https://github.com/KUAILESHANGWEI/sing-box-yg/releases/download/v$upcore/$sbname.tar.gz
if [[ -f '/etc/s-box/sing-box.tar.gz' ]]; then
tar xzf /etc/s-box/sing-box.tar.gz -C /etc/s-box
mv /etc/s-box/$sbname/sing-box /etc/s-box
rm -rf /etc/s-box/{sing-box.tar.gz,$sbname}
...
service iptables save >/dev/null 2>&1
green "Sing-box卸载完成！"
blue "欢迎继续使用Sing-box-yg脚本：bash <(curl -Ls https://raw.githubusercontent.com/KUAILESHANGWEI/sing-box-yg/main/sb.sh)"
echo
}
...
bash <(curl -Ls https://raw.githubusercontent.com/KUAILESHANGWEI/sing-box-yg/main/vendor/CFwarp.sh)
}
bbr(){
...
bash <(curl -Ls https://raw.githubusercontent.com/KUAILESHANGWEI/sing-box-yg/main/vendor/bbr.sh)
fi
}
...
curl -L -o /etc/s-box/sbwpph -# --retry 2 --insecure https://raw.githubusercontent.com/KUAILESHANGWEI/sing-box-yg/main/sbwpph_$cpu
...
echo
blue "sing-box-yg脚本项目地址：https://github.com/KUAILESHANGWEI/sing-box-yg"
echo
blue "推荐甬哥新品：ArgoSBX一键无交互小钢炮脚本"
blue "ArgoSBX项目地址：https://github.com/KUAILESHANGWEI/argosbx"
echo
}
...
echo -e "${bblue}     ░█${plain}█          ░██ ██ ██         ░██  ░░${red}██     ░██  ░░██     ░██  ░░██ ${plain}  "
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" 
white "甬哥Github项目  ：github.com/KUAILESHANGWEI"
...
latestV=$(curl -sL https://raw.githubusercontent.com/KUAILESHANGWEI/sing-box-yg/main/version | awk -F "更新内容" '{print $1}' | head -n 1)
...
echo -e "${yellow}$(curl -sL https://raw.githubusercontent.com/KUAILESHANGWEI/sing-box-yg/main/version)${plain}"
fi
else
